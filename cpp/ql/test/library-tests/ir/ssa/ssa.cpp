...
int PerfFallback(int i) {
  // The size of this array pushes its "expense" metric past the threshold, which turns all memory
  // accesses to the array to just access an unknown offset within the array.
  int a[] = {
  ...
  return a[0] + a[5] + a[63];  // The definitions of these loads are not exact due to the performance fallback
}
